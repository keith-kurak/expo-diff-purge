...
# params:
# 1: SDK version (whole number or something compatible with npx create-expo-app, e.g., "49")
# 2: release version (semver-compatible version to be used for release comparison, e.g., "49.0.0". Usually the same as what is in versions endpoint)
...
AppName=ExpoDiffApp
BundleIdentifier=com.expodiffapp
...
function guardMissingArg () {
    if [ "$#" -ne 2 ]; then
        echo "SDK version or release argument missing."
...
function generateNewReleaseBranch () {
    ...
    cd ExpoDiffApp
    cp app.json app.json.temp && jq '.expo.ios.bundleIdentifier |= "com.expodiffapp" | .expo.android.package |= "com.expodiffapp"' app.json.temp  > app.json && rm -f app.json.temp
